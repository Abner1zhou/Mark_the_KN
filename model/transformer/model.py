...
from model.transformer.layers import Encoder, Decoder


"""
class Transformer(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
                 target_vocab_size, max_pe, rate=0.1):
        super(Transformer, self).__init__()

        self.encoder = Encoder(num_layers, d_model, num_heads, dff,
                               input_vocab_size, max_pe, rate)

        self.decoder = Decoder(num_layers, d_model, num_heads, dff,
                               target_vocab_size, max_pe, rate)
...
if __name__ == '__main__':
    ...
    print("Transformer sample output shape: {}".format(fn_out.shape))  # (batch_size, tar_seq_len, target_vocab_size)(64, 26, 8000)
