...
def text_cnn(max_sequence_length, max_token_num, embedding_dim, output_dim, model_img_path=None, embedding_matrix=None):
    """
    TextCNN:
    1. embedding layers,
    2.convolution layer,
    ...
    logging.info("x_input.shape: %s" % str(x_input.shape))  # (?, 200)
    ...
    logging.info("x_emb.shape: %s" % str(x_emb.shape))  # (?, 200, 300)
